...
**Компилятор** генерирует машинный код **ДО** запуска. Информация об ошибках появится ещё **на этапе сборки**
**Интерпретатор** генерирует байт-код, который последовательно **ВЫПОЛНЯЕТ**. Информация об ошибках появляется постепенно **во время запуска**
