...
- [Чем отличаются компилятор и интерпретатора](#чем-отличаются-компилятор-и-интерпретатор)
...
### Компилятор:
- **преобразует весь исходный код** в машинный код/байт-код заранее
- **результатом** является исполняемый файл или промежуточное представление
- **_примеры:_** Pascal, C, C++, C#, Java, Go

### Интерпретатор:
- **выполняет исходный код построчно** во время запуска программы
- **не создаёт отдельного исполняемого файла**
- **_примеры:_** JavaScript, PHP, Python


|Характеристика|Компилятор|Интерпретатор|
|:-|:-|:-|
|Преобразование кода|перевод в машинный/байт код до выполнения|построчное выполнение|
|Выходной файл|создаёт|нету|
